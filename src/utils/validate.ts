 ...
 * @param email 待测试的邮箱
 */
export const isEmail = (email: string) => /^[A-Za-z0-9]+([.+-_][a-zA-Z0-9]+)*@[A-Za-z0-9]+([.-][a-zA-Z0-9]+)*\.[A-Za-z]{2,14}$/.test(email);
 ...
 * @param nickname 用户昵称
 */
export const isNickName = (nickname: string) => /[A-Za-z0-9_\-\u4e00-\u9fa5]+/.test(nickname);
...
export function isNumber(data: string | number) {
...
export function isMoney(data: string | number) {
  if (typeof data === 'number')
    return /^([1-9]\d{0,9}|0)([.]?|(\.\d{1,2})?)$/.test(data.toString());
  return /^([1-9]\d{0,9}|0)([.]?|(\.\d{1,2})?)$/.test(data);
}

export function isInteger(data: string | number) {
  if (typeof data === 'number')
