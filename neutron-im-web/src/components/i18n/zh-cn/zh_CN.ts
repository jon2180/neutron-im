const zh_CN = {
  'menu.management': '管理',
  'menu.activities': '动态',
  'menu.codesnips': '代码速览板',
  'menu.im': '即时通讯',
  'menu.editor': '编辑器',
  'menu.accountCenter': '个人中心',
  'menu.profileSettings': '个人设置',
  'menu.logout': '退出登录',
  'menu.changeLocale': '切换语言',

  // editor
  /// editor prompted message
  'editor.message.no_title_or_content': '请输入标题和内容',
  'editor.message.invalid_license': '无效的 License',
  'editor.message.empty_original_article_url': '无效的原文链接',
  'editor.message.upload_failed': '上传失败',
  'editor.message.upload_successfully': '上传成功',
  'editor.message.confirm_go_to': '确定要离开吗，确定后你的内容会丢失',

  /// editor: about form label
  'editor.form.input_title': '请输入标题',
  'editor.form.copyright': '版权',
  'editor.form.tags': '标签',
  'editor.form.type': '类型',
  'editor.form.publish': '发布',

  /// editor: form
  'editor.label.is_original': '原创',
  'editor.label.license': '授权',
  'editor.label.original_url': '原文链接',

  /// editor: actions
  'editor.actions.add_tags': '添加标签',

  // im
  'im.menu.recent': '最近',
  'im.menu.contacts': '联系人',
  'im.menu.collections': '收藏',

  // friend
  'friend.addNewFriend': '添加新朋友',
  'friend.title': '朋友 - {username}',

  // chat
  'chat.title': '聊天 - {username}',


  //
  start_record: '开始录制',
  stop_record: '停止录制',
  search: '搜索',
  more: '更多',
  userinfo: '用户信息',
  loadmore: '加载更多',

  'alt.captcha': '验证码',

  // login/register
  'app.login': '登录',
  'app.login.submit': '登录',
  'app.login.greeting': '欢迎来到',
  'app.login.logging': '正在登录...',
  'app.login.logged': '已登录',
  'app.login.form.invalid_account_password_format': '账号或密码格式不符合规则',
  'app.login.unmatchedRepeatPassword': '两次密码输入不一致',
  'app.register': '注册',
  'app.register.submit': '注册',
  'app.register.registered': '注册成功',
  'app.fetchUserInfo.failed': '获取用户信息失败',
  'accounts.settings.basicSettings': '基础设置',
  'accounts.settings.accountSettings': '账号设置',
};

export default zh_CN;
